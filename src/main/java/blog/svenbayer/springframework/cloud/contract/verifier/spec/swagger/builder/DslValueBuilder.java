...
public final class DslValueBuilder {
	...
	private final PatternBuilder patternBuilder = new PatternBuilder();
	 ...
	 */
	public DslProperty<Object> createDslValueForParameter(
			...
			if (ignore != null && Boolean.valueOf(ignore.toString())) {
				if (param.getRequired()) {
					throw new SwaggerContractConverterException(
							"Set the parameter '" + param.getName()
									+ "' to required: false to use x-ignore: true");
				}
				return null;
			}
		}
		...
		if (value == null) {
			value = this.defaultValues.createDefaultValueForType(type, format, name,
		...
		if (!isPatternMatches) {
			throw new SwaggerContractConverterException("The pattern '"
					+ pattern.pattern() + "' does not match for the value '" + value
					+ "' for the given param '" + param.getName() + "'");
		}
	...
	private Object createServerValueForParameter(AbstractSerializableParameter param) {
		...
		if (param.getVendorExtensions() != null
				&& param.getVendorExtensions().get(X_EXAMPLE.field()) != null) {
