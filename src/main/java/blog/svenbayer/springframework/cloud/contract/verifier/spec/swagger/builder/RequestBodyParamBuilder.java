...
public final class RequestBodyParamBuilder {
	 ...
	 */
	public String createValueForRequestBody(BodyParameter param,
			Map<String, Model> definitions) {
		if (param.getExamples() != null && !param.getExamples().values().isEmpty()
				&& param.getExamples().values().toArray()[0] != null) {
			return String
					.valueOf(param.getExamples().entrySet().iterator().next().getValue());
		}
		else if (param.getVendorExtensions() != null && param.getVendorExtensions()
				.get(SwaggerFields.X_EXAMPLE.field()) != null) {
			return String.valueOf(
					param.getVendorExtensions().get(SwaggerFields.X_EXAMPLE.field()));
		}
		else if (param.getSchema() != null) {
			if (param.getSchema().getExample() != null) {
				return String.valueOf(param.getSchema().getExample());
			}
			else if (param.getSchema().getVendorExtensions() != null && param.getSchema()
					.getVendorExtensions().get(SwaggerFields.X_EXAMPLE.field()) != null) {
				return String.valueOf(param.getSchema().getVendorExtensions()
						.get(SwaggerFields.X_EXAMPLE.field()));
			}
			else if (param.getSchema().getReference() != null) {
				String reference = param.getSchema().getReference();
				SwaggerReferenceResolver swaggerReferenceResolver = this.refFactory
						.getReferenceResolver(reference, param.getVendorExtensions());
